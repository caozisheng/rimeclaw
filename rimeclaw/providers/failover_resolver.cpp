...
FailoverResolver::Resolve(const std::string& model,
  ...
  for (const auto& fallback_model : chain_snapshot) {
    ...
    if (result) {
      ...
      spdlog::warn("Primary model '{}' unavailable, falling back to '{}'",
                   model, fallback_model);
      return result;
    }
  }

  spdlog::error("All models exhausted (primary='{}', {} fallbacks)",
                 model, chain_snapshot.size());
  return std::nullopt;
}
...
void FailoverResolver::RecordFailure(const std::string& provider_id,
  ...
  cooldown_.RecordFailure(cooldown_key(provider_id, profile_id), kind,
                          retry_after_seconds);
  spdlog::warn("Provider {}:{} failed ({}), cooldown set{}",
               provider_id, profile_id, ProviderErrorKindToString(kind),
...
FailoverResolver::try_resolve_model(const std::string& model,
  ...
  if (prof_it != profiles_.end() && !prof_it->second.empty()) {
    ...
    if (!cooled_down.empty()) {
      ...
      if (cooldown_.TryProbe(probe_key)) {
        spdlog::info("Probing cooled-down profile {}:{} (probe throttle)",
    ...
    spdlog::debug("All profiles for provider '{}' are in cooldown", provider_id);
    return std::nullopt;
  }
  ...
  if (cooldown_.IsInCooldown(key)) {
    ...
    if (cooldown_.TryProbe(key)) {
      spdlog::info("Probing cooled-down provider '{}' (probe throttle)", provider_id);
