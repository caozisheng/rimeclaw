...
struct CompactionOptions {
  ...
  int max_stages = 4;                   // Maximum number of stages
  int keep_recent = 6;                  // Keep this many recent messages
};
...
class MultiStageCompaction {
 public:
  ...
  // Merge summaries with recent messages.
  static std::vector<Message>
  MergeSummaries(const std::vector<std::string>& summaries,
