...
void MemorySearch::IndexDirectory(const std::filesystem::path& dir) {
  ...
  spdlog::info("Indexed {} entries from {}", entries_.size(), dir.string());
}
...
void MemorySearch::BuildVectorIndex() {
  if (!embedding_provider_) {
    spdlog::warn("No embedding provider set, cannot build vector index");
    return;
  }
  ...
  if (resp.embeddings.size() != entries_.size()) {
    spdlog::error("Embedding count mismatch: got {} for {} entries",
  ...
  spdlog::info("Built vector index with {} entries", vector_index_.Size());
}
...
MemorySearch::score_entry(const IndexEntry& entry,
  ...
  // BM25 scoring: score(D, Q) = Sum IDF(qi) * (f(qi,D) * (k1+1)) / (f(qi,D) +
  // k1*(1-b+b*|D|/avgDL))
  double score = 0;
