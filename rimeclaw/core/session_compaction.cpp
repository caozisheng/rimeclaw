...
SessionCompaction::Compact(const std::vector<nlohmann::json>& messages,
  ...
  if (summary_fn) {
    try {
      ...
      spdlog::error("Compaction summary failed: {}", e.what());
  ...
  spdlog::info("Compacted {} messages into summary + {} recent", to_summarize, keep);
  return result;
}
...
SessionCompaction::Truncate(const std::vector<nlohmann::json>& messages,
  ...
  spdlog::info("Truncated {} messages, keeping {} recent", total - keep, keep);
  return result;
}
