...
MemoryManager::MemoryManager(const std::filesystem::path& workspace_path)
  ...
  // Expected: ~/.rimeclaw/agents/{agentId}/workspace
  // Or legacy: ~/.rimeclaw/workspace
  ...
  spdlog::info("MemoryManager initialized with workspace: {}",
...
void MemoryManager::LoadWorkspaceFiles() {
  spdlog::info("Loading workspace files from: {}", workspace_path_.string());
  ...
  for (const auto& name :
    ...
    try {
      ...
      spdlog::debug("No {} found: {}", name, e.what());
    }
  }
