...
std::vector<Message> MultiStageCompaction::MergeSummaries(
    const std::vector<std::string>& summaries,
    const std::vector<Message>& recent_messages) {
  ...
  if (!summaries.empty()) {
    ...
    result.push_back(Message{"assistant", merged});
  }
...
MultiStageCompaction::CompactMultiStage(const std::vector<Message>& messages,
  ...
  if (!summary_fn) {
    spdlog::warn("No summary function provided, returning messages as-is");
    return messages;
  }
  ...
  if (msg_count < opts.min_messages_for_multistage ||
    ...
    spdlog::info("Single-pass compaction: {} messages, ~{} tokens",
  ...
  spdlog::info("Multi-stage compaction: {} messages split into {} chunks (~{} tokens total)",
  ...
  for (int i = 0; i < num_chunks; i++) {
    ...
    spdlog::debug("Summarizing chunk {}/{}: {} messages, ~{} tokens",
                   i + 1, num_chunks, chunks[i].size(), chunk_tokens);
  ...
  if (opts.target_tokens > 0 &&
    ...
    spdlog::info("Merged summaries (~{} tokens) exceed target ({}), running final summarization pass",
  ...
  spdlog::info("Multi-stage compaction complete: {} -> 1 message", msg_count);
